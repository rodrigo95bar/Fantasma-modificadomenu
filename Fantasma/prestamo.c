...
int prestamo_alta(Prestamo array[], int size, int* contadorID)                          //cambiar prestamo
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            printf("\n Posicion: %d\n ID: %d\n  fechaPrestamo: %s\n ",
                   posicion, array[posicion].idPrestamo,array[posicion].fechaPrestamo);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for (i = 1; i < size; i++)
        {
           ...
           // strcpy(bufferLongString,array[i].varLongString);          //cambiar campo varLongString
            ...
            j = i - 1;
            while ((j >= 0) && strcmp(bufferString,array[j].fechaPrestamo)<0)         //cambiar campo fechaPrestamo                 //Si tiene mas de un criterio se lo agrego, Ej. bufferInt<array[j].varInt
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n fechaPrestamo: %s\n",
                       array[i].idPrestamo,array[i].fechaPrestamo);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
